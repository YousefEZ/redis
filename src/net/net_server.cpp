...
FileDescriptor setup_listener(const sockaddr_in& address)
{
    ...
    SPDLOG_INFO("[SERVER][SETUP][LISTENER] Created socket with fd: {}",
                static_cast<int>(fd));

    int opt = SOCKET_ENABLE;
    setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

    SPDLOG_INFO("[SERVER][SETUP][LISTENER] Binding to {}:{}",
    ...
    SPDLOG_INFO("[SERVER][SETUP][LISTENER] Bound and listening");
    return fd;
}

void run_poll(std::vector<pollfd>& polls)
{
    SPDLOG_DEBUG("[SERVER][POLL][RUN] polling {} fds", polls.size());
